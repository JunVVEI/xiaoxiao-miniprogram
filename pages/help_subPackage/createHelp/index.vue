<template>
	<view class="container">
		...
		<view class="main">
			<!-- 帖子内容 -->
			<view class="content">
				<u--textarea v-model="taskInfo.content" :placeholder="placeholder" :showConfirmBar='false'
		...
		<view class="moneyInput">
			<u-input placeholder="设置金额" border="none" clearable fontSize="22" type="digit" margin="0 3px 0 0"
				v-model="taskInfo.bounty"><u-text :text="text" slot="prefix" margin="0 3px 0 0" size="16"
					type="tips"></u-text>
			</u-input>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			if (this.type == 1) {
				this.text = "悬赏金额"
				this.placeholder = "请描述任务详情"
				uni.setNavigationBarTitle({
					title: "发布悬赏"
				})
			} else {
				this.text = "价格"
				this.placeholder = "请描述物品详情"
				uni.setNavigationBarTitle({
					title: "发布闲置"
				})
			}
		...
		methods: {
			...
			submitHelp() {
				...
				if (this.type == 1) {
					...
					if(!uni.$u.test.amount(this.taskInfo.bounty)){
						uni.showToast({
							icon:"none",
							title:"请输入正确的金额格式"
						})
					...
					addTask(this.taskInfo)
						.then((res) => {
							...
							uni.navigateBack({
								delta: 1,
								success: function() {
									beforePage.$vm.getSecondHandList(true);; // 执行前一个页面的刷新 
								}
							})
						})
						.catch((err) => {

	...
	<view class="container">
		...
		<view class="header">
			<view class="topInfo">
				...
				<view class="leftInfo">
					<!-- 头像 -->
					<u-avatar :src="otherInfo.avatar" shape="circle" size='120rpx'
				...
				<view class="follow-btn">
					<follow-btn :relType.sync="otherInfo.relType" :unfollowBgColor="'#3c9cff'"
						:query="{identityId: otherInfo.identityId, type: otherInfo.type}"
						:fromFansPage="false"></follow-btn>
				</view>
			</view>
			<!-- 数据区 -->
		...
		<view class="main">
			<view class="tab" :style="{width:tabsWidth}">
				<u-tabs ref="uTabs" name="name" :list="tabsList" :is-scroll="false" :current="currentIndex"
					active-color="#2599ff" inactive-color="#606266" font-size="30" height="40px">
					<view slot="right" style="position: relative;right: 30rpx;top: 2rpx;" @tap="showToast">
					</view>
				</u-tabs>
			</view>
					...
					<scroll-view scroll-y @scrolltolower="getMorePost">
						<u-list v-show="postList.length" v-if="postList.length">
							<u-list-item v-for="(item, index) in postList" :key="item.id">
								...
								<view style="height: 470rpx;text-align: center;color: #666;" v-if="index === postList.length-1">
									没有更多了~
								</view>
							</u-list-item>
						</u-list>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			let queryData = JSON.parse(option.queryData);
			getUserPublicInfo(queryData).then((res) => {
				this.otherInfo = Object.assign(res.data.data, queryData)
				console.log(this.otherInfo)
				this.postQuery = {
					currentPage: 1,
					pageSize: 20,
					orderFlag: 2,
					identityId: queryData.identityId
				}
				this.getPost()
				this.query = {
		...
		methods: {
			...
			toHisFollow() {
				...
				// }else{
				uni.navigateTo({
					url: `/pages/mine_subPackage1/follow/index?followTitle=Ta的关注&uid=${uid}`
				})
				// }
